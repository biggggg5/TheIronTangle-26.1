...
public class CalculatorMenu extends AbstractContainerMenu {

    public final DestinationCalculatorBlockEntity blockEntity;
    private final Level level;

    public CalculatorMenu(int containerId, Inventory inv, FriendlyByteBuf extraData) {
        this(containerId, inv, inv.player.level().getBlockEntity(extraData.readBlockPos()), new ItemStacksResourceHandler(1));
    }

    public CalculatorMenu(int containerId, @Nullable Inventory inv, BlockEntity blockEntity, ItemStacksResourceHandler handler) {
        ...
        addSlot(new ResourceHandlerSlot(handler, handler::set, 0, 140, 16) {
    ...
    public int getFuelLevel(){
     return blockEntity.inventory.getAmountAsInt(0) * 19/16;
    }

   // credit to kaupenjoe and diesieben07 | https://github.com/diesieben07/SevenCom
    ...
    public boolean stillValid(Player player) {
        return stillValid(ContainerLevelAccess.create(level, blockEntity.getBlockPos()),
                player, ModBlocks.DESTINATION_CALCULATOR.get());
    }
